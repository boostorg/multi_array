...
namespace boost {
    ...
    namespace multi_array_types {
        ...
        typedef detail::multi_array::extent_range<
            index
          , size_type
        > extent_range;
...
#if !defined(BOOST_MULTI_ARRAY_NO_GENERATORS)
    namespace {
...
namespace detail { namespace multi_array {
    ...
    // Due to some imprecision in the C++ Standard, MSVC 2010 is broken in
    // debug mode: it requires that an Output Iterator have
    // output_iterator_tag in its iterator_category if that iterator is not
     ...
     protected:
            ...
            ) const
        {
            ...
            for (size_type i = 0; i != NumDims; ++i)
            {
                ...
                BOOST_ASSERT(
                    size_type(indices[i] - index_bases[i]) < extents[i]
                );
            }
#endif
            ...
            index offset = 0;
            {
                ...
                for (size_type n = 0; n != NumDims; ++n, ++i)
                {
                    offset += (*i) * strides[n];
                }
            }
        ...
        void
            compute_strides(
              ...
              , general_storage_order<NumDims> const& storage
            )
        {
        ...
        index
            calculate_origin_offset(
                StrideList const& stride_list
              , ExtentList const& extent_list
              , general_storage_order<NumDims> const& storage
              , BaseList const& index_base_list
            )
        {
            return calculate_descending_dimension_offset(
                stride_list
              , extent_list
              , storage
        ...
        template <typename StrideList, typename ExtentList>
        index
            calculate_descending_dimension_offset(
                StrideList const& stride_list
              , ExtentList const& extent_list
              , general_storage_order<NumDims> const& storage
            )
            ...
            ) const
        {
            ...
            for (size_type n = 0; n != NumDims; ++n)
            {
                ...
                index const default_start = index_bases[n];
                index const default_finish = default_start + extents[n];
                ...
                // [start, finish) (with stride) which faces upward when
                // stride is positive and downward when stride is negative,

                // RG: The following code for calculating length suffers from
                // some representation issues: if finish-start cannot be
                // represented as by type index, then overflow may result.

                index len;

                if ((finish - start) / stride < 0)
                {
                    // [start, finish) is empty according to the direction
                    // imposed by the stride.
                    len = 0;
                }
                else
                {
                ...
                // finish marks the open side of the range, so it can go
                // one past the "far side" of the range (the top if stride
                // is positive, the bottom if stride is negative).
                ...
                if (!current_range.is_degenerate())
                {
                    ...
                    // strides for the array_view (see [Garcia] for the
                    // math involved).
