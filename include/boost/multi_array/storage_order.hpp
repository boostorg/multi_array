     ...
     public:
        ...
        general_storage_order(OrderingIter ordering, AscendingIter ascending)
        {
            boost::detail::multi_array::copy_n(
                ordering
              , NumDims
              , this->ordering_.begin()
            );
            boost::detail::multi_array::copy_n(
                ascending
              , NumDims
              , this->ascending_.begin()
            );
        }

        // RG - ideally these would not be necessary, but some compilers
        // don't like template conversion operators.  I suspect that not
        // too many folk will feel the need to use customized storage_order
        // objects, I sacrifice that feature for compiler support.
        general_storage_order(c_storage_order const&)
        {
            for (size_type i=0; i != NumDims; ++i)
            {
                this->ordering_[i] = NumDims - 1 - i;
            }

            this->ascending_.assign(true);
        }

        general_storage_order(fortran_storage_order const&)
        {
            for (size_type i=0; i != NumDims; ++i)
            {
                this->ordering_[i] = i;
        ...
        {
            return this->ordering_[dim];
        }

        bool ascending(size_type dim) const
        {
            return this->ascending_[dim];
        }
        ...
        {
            return (this->ordering_ == rhs.ordering_) && (
                this->ascending_ == rhs.ascending_
            );
        }
    ...
    class c_storage_order 
    {
     ...
     public:
...
#if !defined(__MWERKS__) // Metrowerks screams "ambiguity!"
        template <std::size_t NumDims>
        operator general_storage_order<NumDims>() const
        {
            ...
            return general_storage_order<NumDims>(
                ordering.begin()
              , ascending.begin()
            );
        }
