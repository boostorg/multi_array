...
// Basic_MultiArray to another.  We assume that they are of the same shape.
//
...
namespace boost { namespace detail { namespace multi_array {
    ...
    class copy_dispatch
    {
     ...
     private:
        ...
        static void copy_array(Array1 const& source, Array2 dest)
        {
            copy_array(source.begin(), source.end(), dest.begin());
        }

        static void copy_array(Element const& source, Element& dest)
        {
            dest = source;
        }
    };
    ...
    void copy_array(Array1& source, Array2& dest)
    {
        BOOST_ASSERT(
            std::equal(
                source.shape()
              , source.shape() + source.num_dimensions()
              , dest.shape()
            )
        );
        // Dispatch to the proper function
        typedef typename Array1::element element_type;
        copy_dispatch<element_type>::copy_array(
            source.begin()
          , source.end()
          , dest.begin()
        );
