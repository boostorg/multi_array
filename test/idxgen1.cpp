...
void check(boost::detail::multi_array::index_gen<NumRanges,NumDims> const&)
{
}

bool operator==(range const& lhs, range const& rhs)
{
    return (lhs.start_ == rhs.start_) && (lhs.finish_ == rhs.finish_) &&
    (lhs.stride_ == rhs.stride_) && (lhs.degenerate_ == rhs.degenerate_);
}
