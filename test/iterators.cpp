...
void assign_if_not_const(Array& A, mutable_array_tag const&)
{
    ...
    typedef typename Array::BOOST_NESTED_TEMPLATE subarray<
        2
    >::type::iterator iterator2;
    typedef typename Array::BOOST_NESTED_TEMPLATE subarray<
        1
    >::type::iterator iterator1;
...
struct ittraits_const
{
    ...
    typedef typename boost::subarray_gen<Array,2>::type
        ::const_reverse_iterator riterator2;
    typedef typename boost::subarray_gen<Array,1>::type
        ::const_reverse_iterator riterator1;
};
...
void construct_iterators(Array&)
{
    ...
    // const iterators constructed from iterators.
    {
        ...
        typename Array::const_iterator ci2 = typename Array::iterator();
...
unsigned int test_iterators(Array& A, IterTraits const&)
{
    ...
    // Reverse Iterator Hierarchy Test
    {
        ...
        {
            for (
                riterator2 ii = (*i).rbegin();
                ii != (riterator2)(*i).rend();
                ++ii
            )
            {
                for (
                    riterator1 iii = (*ii).rbegin();
                    iii != (riterator1)(*ii).rend();
                    ++iii
                )
                {
                    BOOST_TEST_EQ(*iii, check_iter_val--);
                }
            }
        }
    }
...
unsigned int access(Array& A, mutable_array_tag const&)
{
    ...
    unsigned int test_runs = test_iterators(A, m_iter_traits());
    test_runs += test_iterators(A, c_iter_traits());

    Array const& CA = A;
    return test_runs + test_iterators(CA, c_iter_traits());
}
...
unsigned int access(Array& A, const_array_tag const&)
{
    ...
    typedef typename ittraits_generator<Array,const_array_tag>::type
        c_iter_traits;
    return test_iterators(A, c_iter_traits());
}
