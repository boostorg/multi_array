...
int main(int, char*[])
{
    ...
    // Construct with nonzero bases
    {
        ...
        double ptr[27];
        array_ref C(ptr, extents[range(1, 4)][range(2, 5)][range(3, 6)]);
        const_array_ref D(
            ptr
          , extents[range(1, 4)][range(2, 5)][range(3, 6)]
        );
        ...
        for (size_type a = 0; a < A.shape()[0]; ++a)
        {
            for (size_type b = 0; b < A.shape()[1]; ++b)
            {
                ...
                {
                    BOOST_TEST_EQ(
                        A[a + bases[0]][b + bases[1]][c + bases[2]]
                      , B[a][b][c]
                    );
                    BOOST_TEST_EQ(
                        C[ a+ bases[0]][b + bases[1]][c + bases[2]]
                      , B[a][b][c]
                    );
                    BOOST_TEST_EQ(
                        D[a + bases[0]][b + bases[1]][c + bases[2]]
                      , B[a][b][c]
                    );
                    // Test that E does not inherit A's index_base
                    BOOST_TEST_EQ(E[a][b][c], B[a][b][c]);
                }
            }
        }
    }
        ...
        for (size_type a = 0; a < A.shape()[0]; ++a)
        {
            for (size_type b = 0; b < A.shape()[1]; ++b)
            {
                for (size_type c = 0; c < A.shape()[2]; ++c)
                {
                    ...
                    BOOST_TEST_EQ(
                        E[a + bases[0]][b + bases[1]][c + bases[2]]
                      , B[a][b][c]
                    );
                }
            }
    ...
    // Set Index Base
    {
        ...
        for (size_type a = 0; a < A.shape()[0]; ++a)
        {
            for (size_type b = 0; b < A.shape()[1]; ++b)
            {
                for (size_type c = 0; c < A.shape()[2]; ++c)
                {
                    BOOST_TEST_EQ(A[a + 1][b + 1][c + 1], B[a][b][c]);
                    BOOST_TEST_EQ(C[a + 1][b + 1][c + 1], B[a][b][c]);
                    BOOST_TEST_EQ(D[a + 1][b + 1][c + 1], B[a][b][c]);
                    BOOST_TEST_EQ(E[a + 1][b + 1][c + 1], B[a][b][c]);
                }
            }
        }
    }
